...
export default function FilterModal({ onClose, trades }: { onClose: () => void; trades: Trade[] }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-[2000] flex items-center justify-center backdrop-blur-md transition-all duration-300" onClick={onClose}>
      <div className="fade-in bg-surface rounded-2xl w-[520px] max-h-[88vh] flex flex-col" onClick={e => e.stopPropagation()}>
        {/* Header */}
        <div className="pt-6 px-6 border-b border-border">
          <div className="flex items-center justify-between mb-6">
            <div className="text-[17px] font-bold text-text">Filters</div>
        ...
        {/* Footer */}
        <div className="py-4 px-6 border-t border-border flex gap-2.5 bg-surface rounded-b-[20px]">
          <button onClick={reset} className="flex-1 p-2.5 bg-surface2 border border-border rounded-lg text-text3 text-[13px] font-medium hover:border-border2 transition-colors">Reset</button>
          <button onClick={apply} className="flex-[2] justify-center p-2.5 text-[13px] btn-dark">Terapkan Filter</button>
        </div>
      </div>
    </div>
  );
