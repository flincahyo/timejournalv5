...
export default function DateRangeModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="flex flex-col h-full bg-bg" onClick={e => e.stopPropagation()}>
      ...
      <div className="flex gap-2 mt-auto pt-6 border-t border-border">
        <button onClick={clear} className="flex-1 btn-outline">Reset</button>
        <button onClick={apply} className="flex-[2] btn-dark">Terapkan</button>
      </div>
    </div>
  );
}
